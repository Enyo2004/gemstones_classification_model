...
# create convolutional model (CNN = Convolutional Neural Network)

convolutionalModel = Sequential([
    ...
    layers.Conv2D( # convolution layer
        filters=64, # features that the layer will look for
        kernel_size=(3,3), # the size of the scanner of the image
        activation='relu', # relu activation function for better generalization
    ),
     

    layers.Conv2D( # convolution layer
        filters=32, # features that the layer will look for
        kernel_size=(3,3), # the size of the scanner of the image
        activation='relu', # relu activation function for better generalization
    ),


    layers.MaxPool2D(pool_size=(2,2)), # Max Pool layers lowers the dimensions with the most important features found in the conv2D layer
    ...
    # global average pooling to get the most important features in the image
    layers.GlobalAveragePooling2D(),

    layers.Dense(len(class_names)), # output layer (output a probability tensor of each class)

    layers.Activation(activation='softmax', dtype=tf.float32), # activation function with float32 for numerical precision
])

# see model's summary to inspect layers
convolutionalModel.summary()


# take a peek at the name, datatype and global policy per layer
for layer in convolutionalModel.layers:
...
# train the model 
convolutionModelHistory = convolutionalModel.fit(
...
evaluation = convolutionalModel.evaluate(validation_data)
print(f"Loss: {evaluation[0]} | Accuracy: {evaluation[1]}") # print the loss and the accuracy


# save the model 
convolutionalModel.save('Models/models/model2/ConvolutionalModel.keras')

# save the weights learned by the model
convolutionalModel.save_weights('Models/models/model2/ConvolutionalModel.weights.h5')
...
plot_loss_curves(convolutionModelHistory) # plot loss curves
